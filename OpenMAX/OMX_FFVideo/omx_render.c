...
static OMX_ERRORTYPE startup_render(RENDER *render)
{
	...
	//portDef.format.image.nSliceHeight = ALIGN16(render->renderConfig->screen_height);
	portDef.format.image.nSliceHeight = 0;
	//portDef.format.image.nStride = ALIGN16(render->renderConfig->screen_width)*4;
...
// 	portDef.format.video.nStride = 0;
	...
	fprintf(stderr, "%s():w=%d h=%d nbuffersize=%d\n", __FUNCTION__, render->renderConfig->screen_width, render->renderConfig->screen_height,
		nBufferSize);
	return OMX_ErrorNone;
}
...
static OMX_ERRORTYPE init_resize(RENDER *render)
{
	...
	portDef.format.image.nFrameWidth = (OMX_U32)srcWidth;
	portDef.format.image.nFrameHeight = (OMX_U32)srcHeight;
	...
	//portDef.nBufferSize = (size_t)srcWidth*srcHeight + (srcWidth*srcHeight) / 2;
	portDef.nBufferSize = ALIGN16(srcWidth)*ALIGN16(srcHeight) * 4;
...
static OMX_ERRORTYPE set_render_display_config(RENDER *render)
{
	...
	//dispRegionConfig.transform = OMX_DISPLAY_MIRROR_ROT0;
...
static OMX_ERRORTYPE resize_port_settings_changed(RENDER *render)
{
	...
	omxPrintPort(render->resizeHandle, render->resizeOutputPort);
	...
	omxPrintPort(render->renderHandle, render->renderInputPort);
	fprintf(stderr, "%s():*********************************\n", __FUNCTION__);
	portDef.format.image.bFlagErrorConcealment = OMX_FALSE;
	portDef.format.image.eCompressionFormat = OMX_IMAGE_CodingUnused;
	portDef.format.image.eColorFormat = OMX_COLOR_Format32bitARGB8888;
...
static OMX_ERRORTYPE close_render(RENDER *render)
{
	...
	ret = OMX_FreeBuffer(render->renderComponent, render->resizeInputPort, render->pInputBuffer);
	if (ret != OMX_ErrorNone){
		fprintf(stderr, "%s() :free render component's input port buffer failed err : %s\n", __FUNCTION__,
			OMX_ErrorToString(ret));
	}
...
static void get_render_display_config(RENDER_DISPLAY_CONFIG *dispConf)
{
	...
	//nBufferSize =ALIGN16(width) * ALIGN16(height) * 4;
	//nBufferSize = width*height*pixel_size;
	//nBufferSize=sign_width * sign_height + (sign_width * sign_height) / 2;
	//nBufferSize = (size_t)srcWidth*srcHeight + (srcWidth*srcHeight) / 2;
	//nBufferSize = (size_t)srcWidth*srcHeight * 4;
	nBufferSize = (size_t)(ALIGN16(srcWidth)*ALIGN16(srcHeight) * 4);
}
...
// 	OMX_ERRORTYPE ret = OMX_SendCommand(render.resizeHandle, OMX_CommandFlush, render.resizeOutputPort, NULL);
// 	if (ret != OMX_ErrorNone){
// 		fprintf(stderr, "%s():send flush resize component command failed err:%s\n", __FUNCTION__,
// 			OMX_ErrorToString(ret));
// 	}
