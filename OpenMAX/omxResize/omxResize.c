...
static void getPorts(OMXResize_s *component) {
	...
	OMX_U32 p;
    for (p = ports.nStartPortNumber; p < pEnd; p++) {
...
void omxResize() {
	...
	uint32_t y;
	uint32_t x;
    for ( y = 0; y < rawImageHeight; y++) {
        for ( x = 0; x < rawImageWidth; x++) {
	...
	setupOutputPort(&ctx.resize, outputFrameSize, OMX_COLOR_FormatYUV420PackedPlanar);
